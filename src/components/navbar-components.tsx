  ...
  return (
    ...
    >
      <motion.p
        ...
        className="cursor-pointer hover:opacity-[0.9] text-white optima"
      >
        {item}
      </motion.p>
      {active !== null && (
...
export const ProductItem = ({ title, description, href, src }) => {
  return (
    <Link href={href} className="flex space-x-2">
      ...
      />
      <div>
        <h4 className="text-xl mb-1 text-white optima">{title}</h4>
        <p className="text-[13px] max-w-[8rem] text-neutral-300 titillium">{description}</p>
      </div>
    </Link>
  );
};
